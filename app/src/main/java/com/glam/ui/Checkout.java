...
import com.glam.adapter.CartListAdapter;
import com.glam.custom.CartManager;
import com.glam.custom.CustomFragment;
import com.glam.custom.SnappyRecyclerView;
import com.glam.custom.WishListManager;
import com.glam.model.Data;
import com.glam.ui.productdetail.ProductDetailsConfig;
import com.glam.ui.productdetail.ProductDetailsConstants;
import com.glam.ui.productdetail.ProductDetailsFragment;
import com.glam.ui.productdetail.ProductDetailsListener;
import com.glam.ui.productdetail.ProductDetailsTheme;
import com.glam.ui.productdetail.ProductDetailsVariantOptionView;
import com.glam.ui.productdetail.ProductImagePagerAdapter;
import com.glam.ui.productdetail.VariantSelectionController;
...
public class Checkout extends CustomFragment
{
	...
	{
		View v = inflater.inflate(R.layout.test_checkout, null);
		myView = v;
...
//		price_total = (TextView)v.findViewById(R.id.price_total);
		recList = (SnappyRecyclerView) v.findViewById(R.id.cardList);
...
//					price_total.setText(new Double(salesPrice).intValue()+"");
					...
					if(activity != null && isAdded()){
						if (CartManager.getInstance().getCart().getSize()>0){
							if (position!=CartManager.getInstance().getCart().getSize()){
								ArrayList<Product> pdLIst1 = ApplicationController.getInstance().getAppPrefs().getObject("D");
								Cart cart1  = CartManager.getInstance().getCart();
								LineItem lineItem1 = cart1.getLineItems().get(position);
								ProductVariant productVariant1 = cart.getProductVariant(lineItem1);
								updateProductDetails(pdLIst1.get(position),productVariant1,position);
							}
							else{
								ArrayList<Product> pdLIst1 = ApplicationController.getInstance().getAppPrefs().getObject("D");
								Cart cart1  = CartManager.getInstance().getCart();
								LineItem lineItem1 = cart1.getLineItems().get(0);
								ProductVariant productVariant1 = cart.getProductVariant(lineItem1);
								updateProductDetails(pdLIst1.get(0),productVariant1,0);
							}
						}else{
							product_details_container.setVisibility(View.GONE);
						}
				...
				if (lastVisiblePosition == -1)
					...
					new Handler().postDelayed(new Runnable() {
						...
						public void run() {
							Activity activity = getActivity();
							if(activity != null && isAdded()){
								updateProductDetails(pdLIst1.get(lastVisiblePosition),pd.get(lastVisiblePosition),lastVisiblePosition);
